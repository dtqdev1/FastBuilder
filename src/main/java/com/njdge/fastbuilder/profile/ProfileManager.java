...
public class ProfileManager {
    ...
    public void loadDefault(PlayerProfile profile) {
        ...
        if (plugin.getMongoManager().isEnabled()) {
            ...
            for (ArenaType type : ArenaType.values()) {
                plugin.getProfileConfig().getConfiguration().set(profile.getUuid().toString() + ".pb." + type.name(), profile.getPbs().get(type));
            }
    ...
    public void load(PlayerProfile profile) {
        ...
        if (plugin.getMongoManager().isEnabled()) {
            ...
            for (ArenaType type : ArenaType.values()) {
                profile.getPbs().put(type, plugin.getProfileConfig().getConfiguration().getLong(profile.getUuid().toString() + ".pb." + type.name()));
            }
    ...
    public void importFromBson(Document document, PlayerProfile profile) {
        ...
        profile.setPbs(document.get("pb", HashMap.class));
    }
}
